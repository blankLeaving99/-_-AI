...
// @storageName 
// @match        *://*/*
// @icon         https://i.imgur.com/3rvrPFQ.jpeg
// @grant        unsafeWindow
...
!!(async function () {
    ...
    class PopupTool {
        ...
        init() {
            ...
            this.apiKeyRow = document.createElement("div");
        ...
        `;
            ...
            /* apiKeyRow: AI备用Key输入 */
            this.apiKeyRow.style.cssText = `padding:4px 8px;display:flex;gap:6px;align-items:center;font-size:13px;border-top:1px solid #e5e7eb;background:#fff;`;
            this.apiKeyInput.type = "password";
            this.apiKeyInput.placeholder = "AI备用Key（题库找不到时自动调用，留空不启用）";
            this.apiKeyInput.value = GM_getValue('lemonApiKey', '');
            this.apiKeyInput.style.cssText = `flex:1;padding:4px 8px;border:1px solid #ccc;border-radius:6px;box-sizing:border-box;font-size:13px;`;
            this.apiKeyRow.appendChild(this.apiKeyInput);

            this.content.append(this.row1, this.apiKeyRow, this.row2, this.row3);
        ...
        bindEvents() {
            ...
            this.saveBtn.onclick = () => {
                GM_setValue('lemonApiKey', this.apiKeyInput.value.trim());
                this.addLog(`Token 保存：${this.tokenInput.value || "(空)"}` + (this.apiKeyInput.value ? '，AI备用Key已保存' : ''));
            };
    ...
    .alert .buttons span:hover {
                ...
                data.onload = function (res) {
                    if (res.responseText.includes('<title>用户登录</title>')) {
                        ...
                        if (isSameDomain(data.url, $l)) {
                            ...
                            if (confirm(`您正在使用的油猴插件版本为测试版：${GM_info.version}\n该版本存在未修复的问题，无法稳定运行此脚本，建议升级或更换油猴版本`)) {
                                $w.location.href = 'https://www.tampermonkey.net/'
                            } else {
                                success(false)
                            }
                        }
        ...
        aiQuery = (questionText, optionsList, questionType) => {
            const apiKey = GM_getValue('lemonApiKey', '');
            if (!apiKey) return Promise.resolve(null);
            const typeMap = { '0': '单选题', '1': '多选题', '2': '填空题', '3': '判断题' };
            const typeLabel = typeMap[String(questionType)] || '选择题';
            let prompt = '请回答以下' + typeLabel + '：\n' + questionText;
            if (optionsList && optionsList.length > 0) {
                prompt += '\n选项：' + optionsList.map((o, i) => String.fromCharCode(65 + i) + '. ' + o).join('  ');
            }
            prompt += '\n\n要求：单选只返回正确选项的完整文字（不含字母），多选返回各正确选项的完整文字用#连接（如选项A内容#选项B内容），判断返回"正确"或"错误"，填空返回答案（多空用#!#分隔）。只返回答案内容，不要字母或任何解释。';
            return new Promise(resolve => {
                GM_xmlhttpRequest({
                    method: 'POST',
                    url: 'https://new.lemonapi.site/v1/chat/completions',
                    headers: { 'Content-Type': 'application/json', 'Authorization': 'Bearer ' + apiKey },
                    data: JSON.stringify({
                        model: '[L]claude-opus-4-6',
            ...
            return new Promise((success, fail) => {
                ...
                async function r(i, success) {
                    if (i >= hostList.length) {
                        confirm('所有服务器均不可用，请稍后刷新重试或尝试更换网络\n请不要使用翻墙软件');
                        fail()
                        return
                    }
                    await brequest({ "url": hostList[i] + 'api/status?version=' + $version, timeout: 3e3 }).then((checkResult) => {
                        if (!checkResult) {
                            ...
                            let l = confirm('当前脚本有新版本，点击确定前往更新');
                            if (l) {
...
<body translate="no">
    ...
    <div id="detail" style="display: none"></div>
            ...
            AIbutton.onclick = function () {
                if (!$w['AIwarning']) {
                    $layer('<center><p>AI答题目前为测试功能</p><p>目前只支持章节测试的AI答题</p><p>不保证AI作答的成绩</p><p>只适用于补充题库没查到的题</p><p>建议关闭自动提交，作答完成后人工检查</p></center>');
                    $w['AIwarning'] = true;
                }
                                    ...
                                    case 'workid':
                                        ...
                                        for (let i = 0, l = questionList.length; i < l; i++) {
                                            try {
                                                ...
                                                if (!tkResultJson || tkResultJson.code != 1) {
                                                    const failMsg2 = tkResultJson?.msg || '题库未找到';
                                                    logs.addLog(failMsg2 + '，尝试AI：' + tm.slice(0, 20), 'orange');
                                                    const aiAns2 = await aiQuery(tm, optionsList, types);
                                                    if (aiAns2) { tkReady = aiAns2; logs.addLog('AI答案：' + aiAns2, 'green'); }
                                                    else { logs.addLog('题库和AI均未找到：' + tm.slice(0, 20), 'red'); continue; }
                                                }
                                                try {
                                                    if (tkResultJson) tkLeft(tkResultJson.left, getTkToken());
                                                    if (tkResultJson && tkResultJson.data && !tkReady) {
    ...
    // 整卷预览页面,在此页面作答
        ...
        async function main() {
            ...
            for (let questionElement of questionsElements) {
                ...
                await waitMinimized()
                if (!ctResult || ctResult['code'] != 1) {
                    addLog('题库未找到，尝试AI：' + question['tm'].slice(0, 20), 'orange');
                    let aiAns = await aiQuery(question['tm'], optionsList, question['questionType']);
                    if (aiAns) {
                        addLog('AI答案：' + aiAns, 'green');
                        if (!ctResult) ctResult = { code: 1, left: $w.left || 0 };
                        ctResult['data'] = aiAns;
                        ctResult['code'] = 1;
                    } else {
                        addLog(question['tm'] + '：题库和AI均未找到', 'red');
                        continue;
                    }
                }
                if (ctResult) {
    ...
    // 作业作答页面
        ...
        async function main() {
            ...
            for (let i = 0, l = tmEs.length; i < l; i++) {
                ...
                await waitMinimized()
                if (!tkResultJson || tkResultJson.code != 1) {
                    const failMsg = tkResultJson?.msg || '题库未找到';
                    addLog(failMsg + '，尝试AI：' + String(question).slice(0, 20), 'orange');
                    let aiAns = await aiQuery(question, optionsList, tmT);
                    if (aiAns) { addLog('AI答案：' + aiAns, 'green'); answer = aiAns; hasAnswer = true; }
                    else { addLog('题库和AI均未找到：' + String(question).slice(0, 20), 'red'); continue; }
                } else if (tkResultJson.data) {
                    answer = tkResultJson.data
                    $w.left = tkResultJson.left
                    popup.setRestCount(tkResultJson.left)
                }
                if (tkResultJson && tkResultJson.left < 1) {
    ...
    if ($l.includes('exam-ans/mooc2/exam/exam-list?') || $l.includes('exam-ans/exam/test/examcode/examnotes?')) {//考试中转页面
        ...
        if (data.done === true) {
            popup.setBaseInfo(`<span style="color:green;">正在阅卷，不会自动作答</span>`)
            ...
            popup.setBaseInfo(`<span style="color:green;">正在单题作答，不会自动切换下一题</span>`)
            $w.gabifujareset = function () {
                data.done = true
                GM_setValue('examData' + examId, data)
            ...
            popup.setBaseInfo(`<span style="color:green;">已答${data.all}题，正确率${vgqtlv}%，正在考试中</span>`)
        }
        GM_setValue('examData' + examId, data)
        await waitMinimized()
        ...
        if (type != -1) {
            ...
            if (!tkResultJson || tkResultJson.code != 1) {
                const failMsg = tkResultJson?.msg || '题库未找到';
                addLog(failMsg + '，尝试AI：' + tm.slice(0, 20), 'orange');
                const aiAns = await aiQuery(tm, realOptions, type);
                if (aiAns) { realAnswer = aiAns; addLog('AI答案：' + aiAns, 'green'); }
                else { addLog('题库和AI均未找到：' + tm.slice(0, 20), 'red'); }
            } else if (tkResultJson.data) {
            ...
            if (tkResultJson && (tkResultJson.left || tkResultJson.left === 0)) {
